...
#define N_INODE_BLOCK    128  // 支持2048个文件 使用128块存储inode 每个block16个inode，所以128*16=2048个inode
...
/**
    ...
    递归查询或创建block
    * @param entry 当前级别索引的addrs指针
    * @param bn 在当前entry块中的索引
    * @param size 当前级别索引所能管理的data_block数量,直接索引为1, 一级间接为ENTRY_PER_BLOCK, 二级间接为ENTRY_PER_BLOCK*ENTRY_PER_BLOCK
 */
static unsigned int locate_block(unsigned int* entry, unsigned int bn, unsigned int size)
{
    ...
    unsigned int next_size = size / ENTRY_PER_BLOCK; // 不同索引级别之间的物理块数目关系
    unsigned int next_bn = bn % next_size; //在下一级索引中的偏移
    unsigned int ret = 0;

    char buf[BLOCK_SIZE];
    block_read(*entry, buf);
    next_entry = (unsigned int*)(buf) + bn / next_size; // 目标数据块属于当前entry块中的第几个下级索引块
    ...
    // 关键修复：如果 locate_block 内部修改了 buf 里的值（即分配了新块），必须写回磁盘
    if (*next_entry != old_val) {
        block_write(*entry, buf); 
    }
    return ret;
}
...
static unsigned int inode_locate_block(inode_disk_t* ip, unsigned int bn)
{
    ...
    if(bn < N_ADDRS_2 * ENTRY_PER_BLOCK)
    {
        ...
        unsigned int idx = bn / size; // 确定在第几个一级间接块
...
int main(int argc, char* argv[])
{
    ...
    block_write(0, buf); // 注意这里的块号是零，不同于xv6的1
    ...
    {
        bn = 0; // change（add)： 每个文件的块号从0开始

        // 确定shortname
        shortname = argv[i] + 7; // 跳过 "./user/"
        assert(*shortname == '_');
    ...
    rooti.size = xint(sizeof(dirent_t) * argc); // 注意. 和 .. 也算在内
